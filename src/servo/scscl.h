...
public:
	...
	int write_pos(uint8_t ID, uint16_t Position, uint16_t Time, uint16_t Speed = 0)
    {
        uint8_t bBuf[6];
	...
	int reg_write_pos(uint8_t ID, uint16_t Position, uint16_t Time, uint16_t Speed = 0)
    {
        uint8_t bBuf[6];
        scserial_->host_2_scs(bBuf+0, bBuf+1, Position);
        scserial_->host_2_scs(bBuf+2, bBuf+3, Time);
	...
	void sync_write_pos(uint8_t ID[], uint8_t IDN, uint16_t Position[], uint16_t Time[], uint16_t Speed[])
    {
	...
	int write_pwm(uint8_t ID, int16_t pwmOut)
    {
        if(pwmOut<0){
            pwmOut = -pwmOut;
            pwmOut |= (1<<10);
